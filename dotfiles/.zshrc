...
export PATH="${HOME}/.local/override/bin:${HOME}/.local/bin:${HOME}/.local/nvim/bin:${HOME}/bin:/usr/local/bin:/usr/local/sbin:/usr/sbin:/usr/bin:/sbin:/bin";
...
function wts() {
  ...
  if [ -n "$TMUX" ] && [ "$use_cd" = false ]; then
    ...
    # Outside tmux or --cd flag: use wt switch in quiet mode to get the path and cd to it
    local target_path
    target_path=$(wt switch --quiet "${args[@]}")
