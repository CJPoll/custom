...
**A design that specifies a destructive migration always raises one** — dropping
a table or a column is gated at merge as an owner decision (`blast-radius` →
`destructive-migration`), and the question it turns on, *does anything still
need this data?*, is the owner's alone. Ask it here, where the answer can be
recorded on the epic days before the merge, rather than leaving the admiral to
stall on it at boarding time.
