...
**Assignee lifecycle — follow the [[athena:ticket-management]] skill**, which
owns it. The `Assignee` always names whoever currently holds the Mission, so
reconcile it on every status move: take scope → **Athena** (the active
connection's bot, resolved via `get-self`, and `Backlog`→`Todo`); dispatch a
captain → `In Progress` (still Athena); any waiting-on-the-human status
(`Needs Attention`, `Done`, work-workspace `Ready for Release`) → **Cody** (for
`Needs Attention` also write the context Cody needs onto the Mission body);
`Attention Given` stays Cody until you resume it (back to `In Progress` →
Athena).
...
- **Pick the engineer's model from the Mission's real complexity** — read the
  Mission and the code it names, not its label. The athena-captain defaults to
  ...
  engineer that goes STUCK is re-dispatched once under Opus into the same
  worktree, which costs more than the tokens Sonnet saved — and **write the
  choice and its one-line reason into the state log** next to the dispatch.
...
start a single `Monitor` (`persistent: true`) on
`~/dev/custom/ai/bin/admiral-report-watch [run-id]` instead of hand-polling on
every turn. That script emits one line per new/updated report file under the
run's reports directory, and forces one staleness heartbeat roughly every 45
minutes even when nothing changed — the heartbeat is what makes the staleness
rule (3b) actually fire on a quiet run instead of only in theory. Run it **only**
under `Monitor` — never via a bare `Bash` call — since its poll loop is the
harness-wakeup path, not a hand-rolled shell spin. When notified — by a report
line or by the heartbeat — do a full sweep per 3b, not just a check of whatever
...
hold real capacity (database memory/connections, containers, volumes, an
address-pool network); leaving them up after a Mission lands is how the box
drifts into pool starvation. **As soon as a Mission's MR is CONFIRMED merged**
(not merely green — a green-but-open MR may still need its stack for review
follow-ups), tear its stack down. Invoke [[athena:teardown-worktree-stack]] for
the per-repo resolution — the project-name derivation is repo-specific and a bare
`down -v` in the wrong repo destroys the wrong stack's volumes, so the three-tier
resolution (repo teardown script → generic default only when the compose project
name is docker's default → documented prose fallback) lives there.

These orchestration rules stay yours and do not move:
- This is the admiral's, not the athena-captain's — they may have terminated
  before the merge happened, and their DONE must not depend on a merge they
  don't perform.
- Only ever tear down stacks belonging to YOUR fleet's Missions, and only after
  ...
  fleet's stack.
- Confirm afterwards (`docker ps -a`, `docker volume ls`, `docker network ls`,
  filtered by the project name) that containers, volumes, and network are gone,
  and record the teardown in the Mission's state-log entry.
...
to **`Needs Attention`**. The first two are yours, computed at merge time; the
third rides the ticket-lifecycle transition and is owned by
**[[athena:ticket-management]]** (its "Needs Attention" DM rule) — not here.
Merging an MR, on its own, pings nobody.

**The trigger is welded to your always-run merge step:** after you merge a
ticket's MR and move that ticket to its terminal status (having CONFIRMED the
merge landed — see "Confirm a merge actually landed"), if the just-merged ticket
has a non-empty `Epic` relation, invoke [[athena:epic-progress-dm]] to recompute
the epic's completion from Notion and DM Cody on any boundary this merge crossed.
A ticket **not** in an epic cannot trigger these — per-epic only. That skill owns
the crossing math (fire once per boundary, recompute from Notion never a running
tally), the Slack mrkdwn formats, the only-a-merge-you-performed rule, and the
decisions-digest shaping under [[athena:run-autonomously]].
