...
bare `wt create`. That default targets `walt_ui`; for any other repo pass it —
`wt-preflight <branch> <repo>` (a bare name resolves under `~/dev/`, e.g.
`wt-preflight dnd-152 gen_saas`), or `--repo <path>`. Do NOT hand-roll a
per-repo copy of this logic — the script places the worktree at
`~/.local/worktrees/<repo>/<branch>` and runs the same staleness checks. `wt create --parent main` branches from the LOCAL `main`, which
