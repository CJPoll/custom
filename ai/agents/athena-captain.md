...
- Check for live children rooted in your own worktree, e.g.
  `pgrep -af -- "$(pwd)"` — never a generic name match. The probe's own
  `zsh -c` shell can appear in that list; it is not a child.
...
- Never `git stash`: every worktree shares the owner's stash list (DND-670; the
  `git-stash-guard` hook denies it). To park WIP, commit it to your worktree branch.
