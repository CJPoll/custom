...
- Every `scripts/test/**/self-test.sh` — DISCOVERED (globbed), never
  ...
  manual wiring step (DND-209: `scripts/test/athena-inbox-client/self-test.sh`,
  42 cases + a sabotage run, shipped by DND-189 and ran automatically nowhere —
  `harness-gate` was 10/10 green and would have stayed green with every
  `scripts/` suite deleted). This is also how `scripts/setup-athena-inbox-client
  --self-test` ends up covered: it runs that same file, declared in the
  runner's `SELF_TEST_DELEGATES` map so it isn't ALSO run as a separate
  installer entry (same suite, one label). `scripts/setup-inbox-registry
  --self-test` delegates the same way, to the already-listed
  `ai/inbox/test/self-test.sh`. The runner prints the discovered count in its
  normal output (`scripts/test self-tests discovered: N`) so a drop to zero is
  visible rather than a silently-shorter PASS list; its `--self-test` proves
  the discovery mechanism itself works — against a throwaway fixture tree,
  never the real `scripts/test/` — by planting a deliberately failing stub,
  confirming it turns red, then fixing/removing it and confirming it turns
  green again; and it FAILS if any `scripts/setup-*` advertising a
  `--self-test` is neither declared nor delegated, so a future installer
  cannot ship an unrun suite either.
