...
- `scripts/setup-hooks --self-test` — the installer's --self-test is INLINE
  (no dedicated `**/test/self-test.sh` file backs it), so it stays a
  hand-declared `CHECKS` entry. Its install / idempotency / merge-safety cases
  are the ONLY verification of the recovery path for the 2026-09-17 hook
  clobber, and were unrun by the gate until DND-209.
- Every `**/test/**/self-test.sh` in the repo — DISCOVERED (globbed), never
  hand-declared, so a future suite anywhere under a `test/` directory is
  ...
  suite shipped as e.g. `scripts/test/foo/tests.sh` is NOT discovered by the
  glob. This patches the CLASS, not one site: DND-209 started from
  ...
  green and would have stayed green with it deleted), and a review pass found
  the same gap already existed for `ai/inbox/test/self-test.sh`,
  `ai/skills/athena:inbox/test/self-test.sh` and
  `ai/skills/athena:slack/test/self-test.sh` — so the glob covers the whole
  repo, not just `scripts/test/`. This is also how
  `scripts/setup-athena-inbox-client --self-test` and `scripts/setup-inbox-registry
  --self-test` end up covered: each runs a file the glob already discovers
  (`scripts/test/athena-inbox-client/self-test.sh`,
  ...
  repo always has committed suites, so zero means the glob broke, not that
  there is nothing to run); it also WARNS (non-blocking, since a helper script
  may legitimately live under a `test/` directory too, and the check does NOT
  filter on the executable bit — a suite committed without `+x` is the likelier
  real mistake) about any other `.sh` file under a `test/` directory not named
  `self-test.sh`, so a misnamed suite is at least visible. Its `--self-test`
  proves the discovery mechanism itself works — against a throwaway fixture
  tree, never a real `test/` directory — by planting a deliberately failing
  stub, confirming it turns red, then fixing/removing it and confirming it
  turns green again, exercising the zero-discovery and stray-suite guards the
  same way, PLUS an end-to-end check that `harness-gate --list` actually lists
  every real discovered suite (catching a regression in the `CHECKS =
  STATIC_CHECKS + self_test_checks` composition itself); and it FAILS if any
  `scripts/setup-*` advertising a `--self-test` is neither declared nor
  delegated, so a future installer cannot ship an unrun suite either.
