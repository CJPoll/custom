...
4. **Diagnose the locus.** For each qualifying pattern, decide the *smallest
   correct* fix using the placement procedure in
   `ai/skills/athena:create-agent-definition/preprocessor.md` ("Blocks vs skills
   vs CLAUDE.md vs template prose"):
...
7. **Commit and journal.** One `git commit` per concern, in `~/dev/custom`, with
   ...
   qualifying patterns are handled.
