...
- `scripts/setup-hooks --self-test` — the installer's --self-test is INLINE
  (no dedicated `scripts/test/**/self-test.sh` backs it), so it stays a
...
- Every `scripts/test/**/self-test.sh` — DISCOVERED (globbed), never
  hand-declared, so a future suite under `scripts/test/` is covered with zero
  manual wiring step, **provided its entry point is named exactly
  `self-test.sh`** — that filename is the whole discovery contract; a suite
  shipped as e.g. `scripts/test/foo/tests.sh` is NOT discovered by the glob
  (DND-209: `scripts/test/athena-inbox-client/self-test.sh`, 42 cases + a
  sabotage run, shipped by DND-189 and ran automatically nowhere — `harness-gate`
  was 10/10 green and would have stayed green with every `scripts/` suite
  deleted). This is also how `scripts/setup-athena-inbox-client --self-test`
  ends up covered: it runs that same file, declared in the runner's
  `SELF_TEST_DELEGATES` map so it isn't ALSO run as a separate installer entry
  (same suite, one label). `scripts/setup-inbox-registry --self-test`
  delegates the same way, to the already-listed `ai/inbox/test/self-test.sh`.
  The runner prints the discovered count in its normal output (`scripts/test
  self-tests discovered: N`) and FAILS outright on zero (this repo always has
  committed suites, so zero means the glob broke, not that there is nothing to
  run); it also WARNS (non-blocking, since a helper script may legitimately
  live there) about any other executable `.sh` under `scripts/test/` not named
  ...
  tree, never the real `scripts/test/` — by planting a deliberately failing
  stub, confirming it turns red, then fixing/removing it and confirming it
  turns green again, PLUS an end-to-end check that `harness-gate --list`
  actually lists the real discovered suite (catching a regression in the
  `CHECKS = STATIC_CHECKS + scripts_self_test_checks` composition itself); and
  it FAILS if any `scripts/setup-*` advertising a `--self-test` is neither
