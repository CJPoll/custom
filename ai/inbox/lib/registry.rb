...
module InboxRegistry
  ...
  # Filename grammar from the contract's "Finding the entry". The filename
  # carries no authority, but a file that fails this grammar is never a
  ...
  def drift(entries = declared)
    ...
    entries.each do |p|
      ...
      rejection = reader_rejection(want)
      problems << "#{file} is declared in a form the athena:inbox reader refuses: #{rejection}" if rejection
      ...
      mode = mode_of(path)
      problems << format("%s has mode %04o, expected %04o", file, mode, ENTRY_MODE) if mode && mode != ENTRY_MODE
  ...
  #
  # Skipped, not failed, where it cannot run (no bash, no jq, skill not
  # present): same environment-safety rule as the rest of this check.
  def skill_lib_dir
    File.join(repo_dir, "ai", "skills", "athena:inbox", "lib")
  end

  def reader_validation_available?
    File.file?(File.join(skill_lib_dir, "descriptor.sh")) &&
      system("command -v jq >/dev/null 2>&1") &&
      system("command -v bash >/dev/null 2>&1")
  end

  # nil when the entry is acceptable (or unvalidatable here); otherwise the
  # reader's own refusal, first line.
  def reader_rejection(entry)
    return nil unless reader_validation_available?
