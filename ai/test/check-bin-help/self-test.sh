...
if [ "${RC}" -eq 0 ] && has "${P:0:12}" && has "pinned"; then
  ok "9b origin moving mid-gate does not redden a check pinned at gate start"
else bad "9b origin moving mid-gate does not redden a check pinned at gate start" "rc=${RC} out=${OUT}"; fi
