...
permission relay with **Slack-confirmed verdicts**, never
`--dangerously-skip-permissions`; (3) **placement A** (the local shim over the
doorbell), not `apps/athena` as the channel server; (4) **close PR #47**
unmerged; (5) **ratify #47's five decisions unchanged** (tier-1 boundary,
sender filter, reply scope, measure-before-tiering, `flock` decides ownership);
(6) filed under a new small epic **"Inbox on Channels"** (`~/dev/custom`,
harness-side) as **DND-281..287**, with **T0 = DND-284** the probe ticket. The
body below is the dated record of the proposal as written on this date;
subsequent build decisions are tracked on the epic and its tickets.
