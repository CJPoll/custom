...
(not `finished`). List the `PARKED` Missions with each resume point. Write this
report BEFORE you release the run with `fleet-resume drained`: after the
release you own nothing ([[athena:fleet-drain]] → *Run ownership*).
